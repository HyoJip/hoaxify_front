...
export const loadOldHoaxes = (hoaxId, username) => {
	const path = username
		? `/api/1.0/users/${username}/hoaxes/${hoaxId}?direction=before&page=0&size=5&sort=id,desc`
		: `/api/1.0/hoaxes/${hoaxId}?direction=before&page=0&size=5&sort=id,desc`;
	return axios.get(path);
};

export const loadNewHoaxes = (hoaxId, username) => {
	const path = username
		? `/api/1.0/users/${username}/hoaxes/${hoaxId}?direction=after&sort=id,desc`
		: `/api/1.0/hoaxes/${hoaxId}?direction=after&sort=id,desc`;
	return axios.get(path);
};

export const countNewHoaxes = (hoaxId, username) => {
	const path = username
		? `/api/1.0/users/${username}/hoaxes/${hoaxId}?direction=after&count=true`
		: `/api/1.0/hoaxes/${hoaxId}?direction=after&count=true`;
	return axios.get(path);
};
